...
public class MySQLiteHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        sql = "create table user(c1 text, c2 text, c3 text, c4 text, c5 text, c6 text, c7 text, c8 text, c9 text, c10 text)";
        db.execSQL(sql);
        insertData(db);
    }
    ...
    public ArrayList<Table> queryData(SQLiteDatabase db) {
        ...
        if (db != null) {
            ...
            while (c.moveToNext()) {
                ...
                String c9 = c.getString(8);
                String c10 = c.getString(9);
                Table table = new Table(c1, c2, c3, c4, c5, c6, c7, c8, c9, c10);
                dataList.add(table);
            }
        }
        return dataList;
